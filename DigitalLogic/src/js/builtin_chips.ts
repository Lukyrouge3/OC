...
export class AndChip extends CustomChip {
    ...
    constructor(x: number, y: number) {
        super(x, y, AndChip.chipColor, AndChip.chipName, [new Pin("A"), new Pin("B")], [new Pin("Out")]);
    }
...
export class NotChip extends CustomChip {
    ...
    constructor(x: number, y: number) {
        super(x, y, NotChip.chipColor, NotChip.chipName, [new Pin("In")], [new Pin("Out")]);
    }

    processOutput(): void {
        this.outputPins.get(0).receiveSignal(this.inputPins.get(0).currentState ? 0 : 1);
    }
}